...
puts "=========== Installing Git"
system('apt-get install git-core -y -qq')
puts "=========== Cloning recipes"
system('rm -Rf /tmp/chef-cookbooks')
system('cd /tmp && git clone git://github.com/redde/chef-rails-production.git chef-cookbooks')
system('cd /tmp/chef-cookbooks')
puts "=========== Running wizard"
...
system('ln -s /var/lib/gems/1.8/bin/chef-solo /bin')
puts "=========== Starting provisioning"
system('chef-solo -c /tmp/chef-cookbooks/solo.rb -j /tmp/chef-cookbooks/node.json -l debug')
